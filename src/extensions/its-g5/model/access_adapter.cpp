...
namespace vcle::itsg5 {
	...
	void AccessAdapter::request(
		const vanetza::dcc::DataRequest& request, std::unique_ptr<vanetza::ChunkPacket> packet
	) {
		if (packet == nullptr) {
			return;
		}
		...
		Device_->Send(
			ToNs3Packet(*packet),
			TypeGlue::convert<ns3::Mac48Address>(request.destination),
			request.ether_type.host()
		);
	}
} // namespace vcle::itsg5
